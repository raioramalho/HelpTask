...
//essa pagina é somente do admin.
    ...
    if (password_verify($_COOKIE['admin'], $_COOKIE['verify'])){
...
<?php
  if(isset($_GET['deleteuser']) == 1){
    $busca = "DELETE FROM `usuario` WHERE `usuario`.`idusuario` = ". $_GET['deleteuser'] ."";
    $result = mysqli_query($con, $busca);
    $row = mysqli_fetch_array($result);
    header("Refresh:0.1, ../panel/homeadmin.php?gerenciar=");
    //print "Veja:". $row[0] ."";
    }
  if(isset($_GET['deletetask']) == 1){
    $busca = "DELETE FROM `chamados` WHERE `chamados`.`idchamado` = ". $_GET['deletetask'] ."";
    $result = mysqli_query($con, $busca);
    $row = mysqli_fetch_array($result);
  ...
  if(isset($_GET['adduser']) == 1){
      $userlogin = htmlspecialchars($_POST['userlogin']);
      $usermail = htmlspecialchars($_POST['usermail']);
      $usersenha = md5(htmlspecialchars($_POST['usersenha']));
      $usersetor = htmlspecialchars($_POST['usersetor']);
      $data = date('y/m/d');
  
      $insertuser = "INSERT INTO `usuario` (`idusuario`, `nome`, `email`, `login`, `senha`, `setor`, `datacadastro`, `dados_status`, `data_exclusao`, `primeira_vez`) VALUES (NULL, '" . $userlogin . "', '" . $usermail . "', '" . $userlogin . "', '" . $usersenha . "', '" . $usersetor . "', '". $data ."', 'ativo', '". $data ."', '0')";
...
<!-- The drawer is always open in large screens. The header is always shown,
  ...
  <div class="mdl-layout__drawer">
    ...
    <nav class="mdl-navigation">
      ...
      <a class="mdl-navigation__link"><i class="material-icons">account_circle</i><?php echo " - Olá: <b>". $_COOKIE['admin'] ."</b>";?></a>
      ...
      <a class="mdl-navigation__link" href="?gerenciar="><i class="material-icons">manage_accounts</i> - Usuários</a>
